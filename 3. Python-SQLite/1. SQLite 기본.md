...
- ## Database 
  ...
  : **정형화**되고 조작 가능한 컴퓨터에 저장된 데이터의 모임 VS 텍스트 파일, 이미지 파일이 그냥 축적된 경우에는 **비정형**데이터라고 지칭합니다.

  : 특정목적을 위한 계산, 저장, 검색, 정렬 등의 '데이터 처리' 작업을 수행

  = 잘 정돈된 창고 (적절한 곳에서 필요한 물건을 찾을 수 있어야 한다.)

  공개 S/W: MySQL, PostgreSQL, MariaDB



  ## 데이터베이스의 기능

  1) 정보를 관리할 수 있는 양식을 제공한다.

  - 데이터 보안성과 안전성 기능 : 데이터베이스가 관리하는 데이터에 대해 자체적인 보안 기능 제공. 권한없는 사용자나 프로그램으로부터 데이터를 보호한다. 또한 컴퓨터 장애나 고장으로부터 데이터를 보호한다.

  2) 데이터 공유 기능 :

  - 데이터 중복의 제거: 데이터를 공유함으로써 비효율성과 일관성 문제를 제거한다.
    - 데이터 무결성(integrity) 유지 : 데이터 무결성은 데이터가 얼마나 정확한가를 뜻한다. **여러 곳에 중복된 데이터 존재하는 경우, 한 곳에 변화가 일어났을 때, 나머지 중복 데이터에 그 변화가 즉각적으로 반영되어야 한다.**
  - 효율적인 자원관리 : 많은 양의 데이터를 다루는데 적합한 효율적이고 효과적인 방법들을 사용한다.
  - 동시성 문제: 다른 프로그램 또는 프로세스가 동시에 같은 데이터에 작업을 하려 할때, 주로 먼저 접근한 프로세스가 일을 마칠 때까지 해당 데이터를 독점하도록 하는 방법으로 문제를 해결한다.



  ## Relational Database 관계형 데이터베이스

  관계형 데이터 모델 : 수학의 관계(relation) 개념을 이용해 데이터를 정의하고 설명한 모델.

  : 테이블 형식을 이용하여 데이터들을 정의하고 설명한 모델

  : 테이블을 릴레이션(relation)이라고 지칭한다.
...
###     데이터 조회
  ...
  ​	앞서서 추가한 데이터를 확인하기 위해서 데이터를 조회 합니다.
  	...
  	SELECT * FROM [테이블 이름]; = 전체 조회, *표시는 전체 컬럼을 조회하겠다는 표시입니다.
​	특정 컬럼만을 조회하고 싶을 때는 SELECT [컬럼명], [추가 컬럼명] FROM [테이블 이름]; 
...
![image-20181107011803124](../misc/resource/image-20181107011803124.png)

```sqlite
SELECT name, team FROM players;
```

![image-20181107011729812](../misc/resource/image-20181107011729812.png)
...
![image-20181107011919096](../misc/resource/image-20181107011919096.png)
...
```sqlite
UPDATE players SET position = 'Midfield'
WHERE name LIKE '%Bernardo%'
OR name LIKE '%Paul%';
```

![image-20181107012246029](../misc/resource/image-20181107012246029.png)
...
![image-20181107012404463](../misc/resource/image-20181107012404463.png)
...
![image-20181107012607662](../misc/resource/image-20181107012607662.png)

![image-20181107012627958](../misc/resource/image-20181107012627958.png)
